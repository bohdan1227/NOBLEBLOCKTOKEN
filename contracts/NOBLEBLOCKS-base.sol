 ...
 * @dev ERC20 Token for NOBLEBLOCKS on BASE
...
contract NOBLEBLOCKS is ERC20, Ownable, ReentrancyGuard {
    ...
    function _mintToken (uint256 _amount) external onlyOwner {
        _mint(owner(), _amount);
    }
    ...
    function _transfer(address from,  address to,  uint256 amount) internal override {
        ...
        require(isBlocked[from] != true && isBlocked[to] != true, "Transfer between blocked address");
        if(isTrade == false && from != owner()) {
            require(from != uniswapV2Pair && to != uniswapV2Pair, "Trade not allowed");
        }
        if(amount == 0) {
    ...
    function setBlock (address _address, bool _value) external onlyOwner {
        require(isBlocked[_address] != _value, "Same value provided");
        isBlocked[_address] = _value;
    }

    function setTrade (bool _value) external onlyOwner {
        require(isTrade != _value, "Same trade value provided");
        isTrade = _value;
    }
}